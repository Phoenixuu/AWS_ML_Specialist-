...
class StudentSCRFD(SCRFD):
    def __init__(self, model_file=None, session=None):
        super().__init__(model_file, session)
        self.fmc = 2
        self._feat_stride_fpn = [8,16]
        self._num_anchors = 1

def distillation_loss(student_logits, teacher_logits, true_labels, T = 2.0, alpha = 0.5):
    """
    Knowledge distillation function/
    student_logits: Predict student model
    teacher_logits: Predict teacher model
    true_labels
    T: Softmax temperature
    alpha: k for Cross-entropy
    """
    #Softmax 
    ...
    #KL Divergence Loss
    kl_loss = F.kl_div(student_probs.log(), teacher_probs, reduction='batchmean') * (T*T)
...
if __name__ == '__main__':
    ...
    teacher_model = SCRFD(model_file="teacher_model.onnx", session=None)
    student_model = StudentSCRFD(model_file="student_model.onnx", session=None)
